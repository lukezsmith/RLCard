...
from rlcard.utils import get_device, set_seed, tournament, reorganize, Logger, plot_curve, tournament_winnings
...
def train(env, algorithm, opposition_agent, cuda, seed, num_episodes, num_eval_games, evaluate_every, save_every, log_dir):
    ...
    eval_env = rlcard.make("teamleducholdem", config={'seed': seed})
    ...
    if opposition_agent == "random":
      # create random agents for evaluation
      opposition_agent_1 = RandomAgent(num_actions=env.num_actions)
      opposition_agent_2 = RandomAgent(num_actions=env.num_actions)
    else:
      opposition_agent_1 = LeducHoldemRuleAgentV2()
      opposition_agent_2 = LeducHoldemRuleAgentV2()
    
    # set agents for training
    env.set_agents(agents)
    # env.set_agents([agents[0], random_agent_1, agents[2], random_agent_2])
    ...
    eval_env.set_agents([agents[0], opposition_agent_1, agents[2], opposition_agent_2])
    ...
    with Logger(log_dir, opposition_agent) as logger:
      ...
      for episode in range(num_episodes):
          ...
          if episode % evaluate_every == 0:
              ...
              # reward = 0
              # for eval_episode in range(num_eval_games):
              #     _, payoffs = eval_env.run(is_training=False)
              #     reward += payoffs[0]

              # logger.log('\n########## Evaluation ##########')
              # logger.log('Timestep: {} Average reward is {}'.format(env.timestep, float(reward)/num_eval_games))

              

              # logger.plot(csv_path, "nfsp")
              # logger.plot('nfsp')
...
if __name__ == '__main__':
    ...
    train( env, algorithm, opposition_agent, cuda, seed, num_episodes, num_eval_games, evaluate_every, save_every, log_dir)
