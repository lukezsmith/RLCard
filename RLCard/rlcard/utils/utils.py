...
def tournament_winnings(env, num):
    ''' Evaluate he performance of the agents in the environment
    ...
    '''
    payoffs = [0 for _ in range(env.num_players)]
    winnings = [0 for _ in range(env.num_players)]
    counter = 0
    while counter < num:
        _, _payoffs, _winners = env.run(is_training=False)
        # print(_winners)
        if isinstance(_payoffs, list):
            for _p in _payoffs:
                for i, _ in enumerate(payoffs):
            ...
            for i, _ in enumerate(_winners):
                winnings[i] += _winners[i]
            counter += 1
    for i, _ in enumerate(payoffs):
        payoffs[i] /= counter
    for i, _ in enumerate(winnings):
        winnings[i] /= counter
    return payoffs, winnings
...
def plot_winrate_curve(csv_path, save_path, algorithm):
    ''' Read data from csv file and plot the results
    '''
    import os
    import csv
    ...
    with open(csv_path) as csvfile:
        ...
        fig, ax = plt.subplots()
        ax.plot(xs, ys, label=algorithm)
        ax.set(xlabel='timestep', ylabel='winrate')
        ax.legend()
        ax.grid()
