...
from rlcard.games.teamleducholdemv2 import Judger
from rlcard.games.teamleducholdemv2 import Round
...
class LeducholdemGame(Game):
    ...
    def get_payoffs(self):
        ...
        winner_reward = 2
        winning_teammate_reward = 0.1

        winner = self.judger.judge_game(self.players, self.public_card)

        winning_team = teams[winner % 2]
        winning_teammate = winning_team[(winning_team.index(winner) + 1) % 2]
        loser_indices = [i for i in range(len(self.payoffs)) if i not in winning_team]
        winners = [1  if i in winning_team else 0 for i in range(self.num_players)]

        # distribute winner rewards
        self.payoffs[winner] = winner_reward
        self.payoffs[winning_teammate] = winning_teammate_reward

        # distribute losing penalty
        for i in loser_indices:
            self.payoffs[i] = - ((winner_reward + winning_teammate_reward) / 2)
