...
class LeducholdemRound:
    ...
    def proceed_round(self, players, action):
        ...
        if action == 'call':
            ...
            self.folded_players.append(self.game_pointer)

            # get player's team length
            team_length = self.team_lengths[self.game_pointer % 2]
            if team_length == 1:
                players[self.game_pointer].status = 'folded'
                self.team_lengths[self.game_pointer % 2] -=1
                winning_team = self.teams[(self.game_pointer + 1) % 2]
    ...
    def is_over(self):
        ...
        """
        if self.is_won or (self.not_raise_num >= self.num_players):
            return True
        return False
