from rlcard.games.teamuno.card import UnoCard
from rlcard.games.teamuno.utils import cards2list, WILD, WILD_DRAW_4
...
class UnoRound:
    ...
    def proceed_round(self, players, action):
        ...
        '''
        # skip if current player is not in the game anymore
        if players[self.current_player].status == 'shed':
        ...
        if not player.hand:
            ...
            # add player to the list of shed players so they can get some reward
            self.shed_players.append(self.current_player)
            # check if other teammate has shed all cards

            # get player's team length
            team_length = self.team_lengths[self.current_player % 2]
            if team_length == 1:
                self.is_over = True
                winning_team = self.teams[self.current_player % 2]
                self.winning_player = self.current_player
                self.winner = winning_team 

            # not finished, remove player from pool
            else:
                # remove player from playing pool
                players[self.current_player].status = 'shed'
                self.team_lengths[self.current_player % 2] -=1
    ...
    def get_player_hands(self, players):
        nums = []
        for i in range(4):
            player = players[i]
            nums.append(len(player.hand))
        return nums
