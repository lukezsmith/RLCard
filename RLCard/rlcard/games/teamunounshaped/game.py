...
from rlcard.games.teamunounshaped import Dealer
from rlcard.games.teamunounshaped import Player
from rlcard.games.teamunounshaped import Round
...
class UnoGame:
    ...
    def get_payoffs(self, algorithm="nfsp", partial_rewards=True):
        ...
        winning_player = self.round.winning_player
        loser_indices = [i for i in range(len(self.payoffs)) if i != winning_player]
        winners = [1  if i in winner else 0 for i in range(self.num_players)  ]

        self.payoffs[winning_player] = winner_reward
